...
begin
  ...
  case FOperacoesSistema of
    ...
    puExcluirProdutosPosImpressao: begin
      ...
      raise Exception.Create('Nenhuma operação de sistema informada.');
  end;
...
begin
  case AType of
    tmProdutoLancado : ToastProdutoLançado;
...
begin
  if (TabControlPrincipal.ActiveTab <> TabItemLogin) and (TabControlPrincipal.ActiveTab <> TabItemMenu) then
    raise Exception.Create('Esse item só pode ser acessado se você estiver na tela de LOGIN ou MENU.');
end;
...
procedure TForm3.ToastProdutoLançado;
begin
...
begin
  case FTabs of
    ...
    tTabMenu:               begin
                              raise Exception.Create(
                                'Para voltar para tela de login, é preciso FECHAR o caixa.'
                              );
                            end;

    tTabGrupos:             begin
                              raise Exception.Create(
                                'Para sair deste pedido é preciso finaliza-lo.'
                              );
                            end;
